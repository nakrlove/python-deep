...
X = dia.sample(frac=0.1)
dia = dia.drop(['cut', 'clarity', 'color'], axis=1)
# 테스트할 클러스터의 수를 리스트로 정의
n_clusters_list = [4,5,6,7,8,]c
...
for i, n_clusters in enumerate(n_clusters_list):
    ...
    labels = km.fit_predict(dia)
    
    # 데이터 포인트별 실루엣 점수 계산
    silhouette_vals = silhouette_samples(dia, labels)
    
    # 전체 평균 실루엣 점수 계산
    avg_score = silhouette_score(dia, labels)
...
for n_clusters in n_clusters_list:
    km = KMeans(n_clusters=n_clusters, random_state=42, n_init='auto')
    labels = km.fit_predict(dia)
    score = silhouette_score(dia, labels)
